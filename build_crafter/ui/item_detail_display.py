...
class ItemDetailDisplay(tk.Frame): # Reste un Frame comme conteneur principal
    ...
    def __init__(self, parent, bg_color="#2B2B2B", fg_color="white", accent_color="#4E9AFA",
                 on_equip_callback=None, *args, **kwargs):
        ...
        self.canvas = tk.Canvas(self, borderwidth=0, background=self.bg_color, highlightthickness=0)
        ...
        # Espacement en haut
        tk.Frame(self.scrollable_frame, height=15, bg=self.bg_color).grid(row=0, column=0, sticky="ew")

        self.name_label = tk.Label(self.scrollable_frame, text="", font=("Segoe UI", 16, "bold"), bg=self.bg_color, fg=self.fg_color)
        ...
        self.equip_button = tk.Button(self.scrollable_frame, text="Équiper", font=("Segoe UI", 10, "bold"), bg="#555555", fg="white", relief=tk.RAISED, bd=2, activebackground="#666666", activeforeground="white", state=tk.DISABLED, cursor="hand2", command=self._handle_equip_click)
    ...
    def _on_frame_configure_debounced(self, event=None):
        if self._update_scroll_job: self.after_cancel(self._update_scroll_job)
        self._update_scroll_job = self.after(self._debounce_ms, self._update_scrollregion)
    def _on_canvas_configure_debounced(self, event):
        if self._update_canvas_width_job: self.after_cancel(self._update_canvas_width_job)
        self._update_canvas_width_job = self.after(self._debounce_ms, lambda w=event.width: self._update_canvas_width(w))
    ...
    def _bind_mousewheel(self, widget):
         widget.bind("<MouseWheel>", self._on_mousewheel, add='+'); widget.bind("<Button-4>", self._on_mousewheel, add='+'); widget.bind("<Button-5>", self._on_mousewheel, add='+')
    def _unbind_mousewheel(self, widget): # Important pour le nettoyage
         try: widget.unbind("<MouseWheel>"); widget.unbind("<Button-4>"); widget.unbind("<Button-5>")
         except tk.TclError: pass
    def _on_mousewheel(self, event):
        if event.num == 4: delta = -1
        elif event.num == 5: delta = 1
        else: delta_val = getattr(event, 'delta', 0); delta = -1 * int(delta_val / 120) if delta_val != 0 else 0
        try: self.canvas.yview_scroll(delta, "units")
        except tk.TclError: pass
        return "break"

    # --- Logique interne (_clear_display, _select_level, etc.) ---
    def _clear_display(self):
        self.item_data = None; self.selected_level = None; self.image_reference = None
        # Vider les frames internes
        for widget in self.level_selector_frame.winfo_children(): self._unbind_mousewheel(widget); widget.destroy()
        for widget in self.details_frame.winfo_children(): self._unbind_mousewheel(widget); widget.destroy()
        # Mettre à jour les labels/boutons principaux (qui restent)
        self.name_label.config(text=""); self.image_label.config(image=""); self.equip_button.config(state=tk.DISABLED)
        self.level_buttons = {}
        # Reset scroll
        try: self.canvas.yview_moveto(0); self._update_scrollregion()
        except tk.TclError: pass


    def _select_level(self, level): # S'assure de mettre à jour la scrollregion après changement
        ...
        for lvl, widgets in self.level_buttons.items():
             button, underline = widgets['button'], widgets['underline']; is_selected = (lvl == level)
             button.config(fg=self.fg_color if is_selected else "#AAAAAA", font=("Segoe UI", 11, "bold" if is_selected else "normal")); underline.config(bg=self.accent_color if is_selected else self.bg_color, height=2)
    ...
    def _populate_levels(self): # S'assure de lier la molette aux nouveaux widgets
        ...
        levels_dict = self.item_data.get('levels', {}); min_level = self.item_data.get('min_level');
        if not levels_dict or min_level is None: return
        ...
        for i, level in enumerate(available_levels):
             ...
             self._bind_mousewheel(level_frame); self._bind_mousewheel(button); self._bind_mousewheel(underline)
    ...
    def _add_detail_row(self, row_index, label_text, value_text): # S'assure de lier la molette
        # ... (création label, value, sep comme avant) ...
         ...
         sep = ttk.Separator(self.details_frame, orient='horizontal'); sep.grid(row=row_index + 1, column=0, columnspan=2, sticky='ew', pady=(5, 5))
         # --- Lier la molette ---
         self._bind_mousewheel(label); self._bind_mousewheel(value); self._bind_mousewheel(sep)
         return row_index + 2
    ...
    def _populate_details(self): # S'assure de lier la molette
        ...
        for widget in self.details_frame.winfo_children(): self._unbind_mousewheel(widget); widget.destroy()
        row = 0; details_title = tk.Label(self.details_frame, text="Details", font=("Segoe UI", 12, "bold"), bg=self.bg_color, fg=self.fg_color, anchor="w"); details_title.grid(row=row, column=0, columnspan=2, sticky="w", pady=(0, 10)); self._bind_mousewheel(details_title); row += 1
        ...
        if children and isinstance(children[-1], ttk.Separator): self._unbind_mousewheel(children[-1]); children[-1].destroy()


    def _handle_equip_click(self): # Inchangé
        if self.item_data and self.on_equip_callback: self.on_equip_callback(self.item_data)
    ...
    def update_display(self, item_data): # Met à jour scrollregion et scroll to top
        ...
        if self.item_data is None or not isinstance(self.item_data, dict):
            self.name_label.config(text="Sélectionnez un item"); self.equip_button.config(state=tk.DISABLED)
            # --- Mise à jour scrollregion même si vide ---
            self.update_idletasks(); self._on_frame_configure_debounced()
        ...
        try: self.canvas.yview_moveto(0) # Remettre le scroll en haut
        except tk.TclError: pass
